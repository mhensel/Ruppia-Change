...
SAV_2020 = vroom::vroom("/Volumes/savshare2/Current Projects/Predicting-SAV/data/communityDFs/SAVCommunityDens_AllStations.csv") %>% 
  group_by(Year) %>% 
  mutate(Baywide = sum(SAVArea, na.rm = F)) %>% 
  group_by(SpCluster, Year) %>%
  summarize(Hectares = sum(SAVArea, na.rm = F), 
            Baywide = mean(Baywide), 
            perBay = Hectares/Baywide)


SAV_time.tbl_2020 = SAV_2020 %>% group_by(SpCluster) %>%
  mutate(Change = Hectares - lag(Hectares, n = 1L)) %>% 
...
baywideRuZoSAV <- 
  ...
  stat_summary(data = SAV, aes(x = year, y = meanRu/1000), color = "#37F918", fun.data = mean_cl_normal, geom = "line", fun.args = list(mult = 1), size = 2) +
  stat_summary(data = SAV, aes(x = Year, y = meanZo/1000), color = "#1b53eb", fun.data = mean_cl_normal, geom = "line", fun.args = list(mult = 1), size = 1.5) +
...
PropBay <- 
  ggplot(data = SAV) + 
  stat_summary(aes(x = Year, y = perBayZo),color = "#1b53eb", fun.data = mean_cl_normal, geom = "line", fun.args = list(mult = 1), size = 2) +
  stat_summary(aes(x = Year, y = perBayRu), color = "#37F918", fun.data = mean_cl_normal, geom = "line", fun.args = list(mult = 1), size = 2)+
  scale_colour_manual(labels = (c("Ruppia", "Zostera")), values=c("#37F918", "#1b53eb")) +
  scale_x_continuous(n.breaks = 10) + 
  ylim(0, .6)+
...
TempBay = 
  ...
  filter(SpCluster %in% c("Zostera", "RuZo")) %>% #Mixed zone and Zo zones
  filter(!year == "1984") %>% ungroup() %>% #outlier year (no sampling done in summer)
    ggplot(data = .) + 
    geom_hline(yintercept = 25, linewidth = 10, color = "darkgrey", alpha = .6) +
     geom_smooth(aes(x = year, y = Temp.summe), method = "lm", se = T, color = "black", linewidth = 1, alpha = .7) +
    stat_summary(aes(x = year, y = Temp.summe), color = "darkgrey", fun.data = mean_cl_normal, 
                 geom = "line", size = 1.5, alpha = .7) +
    #stat_summary(aes(x = year, y = Temp.summe, ymax = Temp.summax, ymin = Temp.summin), fun.data = mean_se, 
   #              geom = "pointrange", size = .5, alpha = .4, color = "dark red") + 

    geom_text(aes(y = 25, x = 1992), label = "Eelgrass Temperature Threshold", color = "white", size = 5) +
    scale_x_continuous(n.breaks = 10) + 
...
ggplot(data = SAV_stability) + 
  stat_summary(aes(x = year, y = long.detredevRu), color = "#37F918", fun.data = mean_cl_normal, geom = "line", fun.args = list(mult = 1), size = 2.3) +
  stat_summary(aes(x = Year, y = long.detredevZo), color = "#1b53eb", fun.data = mean_cl_normal, geom = "line", fun.args = list(mult = 1), size = 2.3) +
    stat_summary(aes(x = Year, y = long.detredevBae), color = "black", fun.data = mean_cl_normal, geom = "line", fun.args = list(mult = 1), size = 1.3, alpha = .8) +
